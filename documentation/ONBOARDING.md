...
- **Player dashboard sidebar is viewport-fixed on desktop** — keep `#desktop-sidebar` fixed in `resources/views/components/layouts/dashboard.blade.php` and preserve the matching desktop content offset so long player pages scroll without creating bottom whitespace under the sidebar action area.
...
| Chat conversations | `chat_conversations`, `chat_participants`, `chat_messages`, `player_follows` | Persisted global, player-to-player direct, and team chat. Messages are delivered over private Reverb channels authorized by conversation participation or active team membership; player profile modals support follow/message actions |
