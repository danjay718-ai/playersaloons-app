<div>
    ...
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-1/3 xl:w-1/4 flex flex-col space-y-2 relative">
            ...
            <div class="flex items-center justify-between mb-2 px-1">
                ...
                <button wire:click="openCreateRole" class="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 transition-colors">
                ...
                <div class="group flex items-center justify-between w-full px-4 py-3.5 rounded-xl border transition-all duration-200 cursor-pointer {{ $activeRoleId === $role->id ? 'bg-indigo-600/10 border-indigo-500/50 shadow-[0_0_15px_rgba(79,70,229,0.1)]' : 'bg-slate-900 border-slate-800 hover:border-slate-700 hover:bg-slate-800' }}"
                    ...
                    <div class="flex items-center gap-1.5 shrink-0 ml-2" @click.stop>
                        @if(!in_array($role->name, \App\Livewire\Admin\RolePermissionAdmin::PROTECTED_ROLES, true))
                            <button wire:click.stop="openEditRole({{ $role->id }})" title="Rename Role" class="p-1 text-slate-500 hover:text-indigo-400 transition-colors">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button wire:click.stop="openDeleteRole({{ $role->id }})" title="Delete Role" class="p-1 text-slate-500 hover:text-red-400 transition-colors">
    ...
    @if($showCreateModal)
        <div x-data="{ open: true }" x-show="open" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div class="fixed inset-0 bg-black/75 backdrop-blur-sm" @click="open = false; $wire.set('showCreateModal', false)"></div>
            <div class="bg-[#0f172a] border border-slate-800 rounded-xl max-w-md w-full overflow-hidden shadow-2xl relative z-10">
                <div class="px-6 py-4 border-b border-slate-800 bg-[#0b0f19] flex justify-between items-center">
                    ...
                    <button type="button" @click="open = false; $wire.set('showCreateModal', false)" class="text-slate-400 hover:text-white">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>

                <form wire:submit.prevent="createRole" class="p-6 space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Role Name</label>
                        <input type="text" wire:model="newRoleName" placeholder="e.g. TOURNAMENT_REFEREE"
                    ...
                    <div class="pt-4 border-t border-slate-800 flex justify-end space-x-3">
                        <button type="button" @click="open = false; $wire.set('showCreateModal', false)" 
    ...
    @if($showEditModal)
        <div x-data="{ open: true }" x-show="open" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="bg-[#0f172a] border border-slate-800 rounded-xl max-w-md w-full overflow-hidden shadow-2xl relative z-10">
                ...
                <form wire:submit.prevent="updateRoleName" class="p-6 space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase mb-1">New Role Name</label>
                        <input type="text" wire:model="editRoleName" placeholder="e.g. SENIOR_ORGANIZER"
                               class="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 uppercase font-mono focus:outline-none focus:border-indigo-500">
                        @error('editRoleName') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div class="pt-4 border-t border-slate-800 flex justify-end space-x-3">
                        <button type="button" @click="open = false; $wire.set('showEditModal', false)" 
                                class="bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs uppercase px-4 py-2.5 rounded-lg transition-colors">
                            Cancel
                        </button>
                        <button type="submit" 
        ...
        <div x-data="{ open: true }" x-show="open" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="bg-[#0f172a] border border-red-500/30 rounded-xl max-w-md w-full overflow-hidden shadow-2xl relative z-10">
                <div class="px-6 py-4 border-b border-slate-800 bg-[#0b0f19] flex justify-between items-center">
                    ...
                    <button type="button" @click="open = false; $wire.set('showDeleteModal', false)" class="text-slate-400 hover:text-white">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>

                @php $delRole = \Spatie\Permission\Models\Role::find($deletingRoleId); @endphp
                <div class="p-6 space-y-4">
                    <p class="text-xs text-slate-300 leading-relaxed">
                        Are you sure you want to permanently delete the role <strong class="text-red-400 font-mono">{{ $delRole?->name }}</strong>?
