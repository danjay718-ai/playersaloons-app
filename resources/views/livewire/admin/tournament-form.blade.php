    ...
    <form x-ref="form" x-on:submit.prevent="save" class="w-full overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/70 shadow-2xl">
        <section data-step="1" x-show="step === 1" x-cloak class="p-5 md:p-8">
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-3">
                ...
                <div><label class="field-label">Game *</label>@if($isLocked && $historicalGame?->trashed())<p class="form-field">{{ $historicalGame->localizedName('en') }}</p>@else<select wire:model.live="game_id" required data-invalid-zero="true" @disabled($isLocked) class="form-field"><option value="0">Select a game</option>@foreach($games as $game)<option value="{{ $game->id }}">{{ $game->translations->first()?->name ?? $game->slug }}</option>@endforeach</select>@endif@error('game_id') <p class="field-error">{{ $message }}</p> @enderror</div>
                @if($isLocked && $historicalPlatformNames !== '')<div><p class="field-label">{{ __('Platforms') }}</p><p class="form-field">{{ $historicalPlatformNames }}</p></div>@else<fieldset><legend class="field-label">{{ __('Platforms') }} *</legend><div class="grid grid-cols-1 gap-2 sm:grid-cols-2">@foreach($platforms as $platform)<label class="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200"><input type="checkbox" wire:model="platform_ids" value="{{ $platform->id }}" @disabled($isLocked) class="rounded border-slate-700 text-indigo-500">{{ $platform->name }}</label>@endforeach</div><p class="mt-2 text-xs text-slate-400">{{ __('Select all platforms players can use for this competition.') }}</p>@error('platform_ids')<p class="field-error">{{ $message }}</p>@enderror @error('platform_ids.*')<p class="field-error">{{ $message }}</p>@enderror</fieldset>@endif
