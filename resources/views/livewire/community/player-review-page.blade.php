<div class="mx-auto w-full max-w-2xl space-y-6">
    ...
    <section class="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-6">
        ...
        <form wire:submit="submit" class="mt-6 space-y-5">
            <div><label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Star rating</label><div class="mt-3 flex gap-2">@for($star=1;$star<=5;$star++)<button type="button" wire:click="$set('rating', {{ $star }})" class="text-3xl {{ $rating >= $star ? 'text-amber-300' : 'text-zinc-700' }}">★</button>@endfor</div></div>
