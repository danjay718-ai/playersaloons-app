                ...
                @if($recentMatches !== [])
                    <div class="grid gap-3 p-4 sm:grid-cols-2">
                        @foreach($recentMatches as $match)
                            <a href="/matches/{{ $match['uuid'] }}" wire:navigate class="rounded-xl border border-zinc-800 bg-zinc-900/55 p-4 transition hover:border-violet-500/30"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate text-xs font-bold text-white">{{ $match['tournament'] }}</p><p class="mt-1 text-[10px] text-zinc-600">{{ $match['game'] }} · {{ ucfirst($match['status']) }}</p></div><span class="rounded-md px-2 py-1 text-[9px] font-black uppercase {{ $match['outcome'] === 'win' ? 'bg-emerald-500/10 text-emerald-300' : ($match['outcome'] === 'loss' ? 'bg-rose-500/10 text-rose-300' : 'bg-amber-500/10 text-amber-300') }}">{{ $match['outcome'] }}</span></div><p class="mt-3 text-[9px] font-black uppercase tracking-wider text-violet-400">Open Match Room · {{ $match['updated_at'] }}</p></a>
                        @endforeach
                    </div>
                @else
